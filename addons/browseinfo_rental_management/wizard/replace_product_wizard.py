...
import odoo.addons.base.models.decimal_precision as dp #import openerp.addons.decimal_precision as dp
...
class RentalProductReplace(models.Model):
    ...
    def replace_product(self):
        ...
        for line in self.existing_product_ids:
            if line.replace_item == True and line.ro_line_id:
                ...
                # creatre incoming recipt for it
                ...
                #     'product_id': line.ro_line_id.product_id.id,
                #     'product_uom': line.ro_line_id.product_id.uom_id.id,
                #     'product_uom_qty': 1,
        ...
        if len(is_replace) == 0:
            ...
            # for line in self.existing_product_ids:
            #     if line.replace_item == True:
            #         for pick_cancel in pick_ids:
            #             # ori_move_obj = self.env['stock.move'].search([('picking_id','=', pick_cancel.id),('for_rental_move','=',True),('product_id','=',line.product_id.id)])
            #             print(pick_cancel.move_lines,"///////////////////////////////////////")
            #             for move in pick_cancel.move_lines:
            ...
            #          'partner_id': rental.partner_id.id or False,
            #          'location_id': rental.partner_id.property_stock_customer.id,
            #          'location_dest_id': pick_ids.location_id.id,
            #          'picking_type_id': pick_type,
            #          'move_lines': move_lines,
            #          'group_id': rental.procurement_group.id,
            #          'rental_ref_id': rental.id}
            # picks = self.env['stock.picking'].sudo().create(vals)
            # picks.group_id = rental.procurement_group.id
    ...
    def replace_product(self):
        ...
        is_replace = []
        move_lines = []
        pick_ids = self.env['stock.picking'].search([('group_id', '=', rental.procurement_group.id)])
        for pick in pick_ids:
            if pick.state != 'done':
                raise UserError(_('You can only replace products those are delivered!'))
        ...
        if len(is_replace) == 0:
            ...
            vals = {'origin': rental.name,
                    ...
                    'group_id': rental.procurement_group.id,
                    'rental_ref_id': rental.id}
            picks = self.env['stock.picking'].create(vals)
            picks.group_id = rental.procurement_group.id
...
class ReplaceExistingProductLine(models.Model):
    ...
    replace_item = fields.Boolean(string='Replace?', default=False)
